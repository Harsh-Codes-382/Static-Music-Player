...
const change = (songs) => {
    ...
    image.src = `assets/image/${songs.Name}.jpg`;
    music1.src = `assets/music/${songs.Name}.mp3`;
}
...
const ran_song = () =>{
    ...
    let x = Math.floor(Math.random()*songs.length);
    change(songs[x]);
    playmusic();
    random_bg();
}
// on loop song
Loop_song.addEventListener('click', Song_loop);

// remove the loop song
shuffle.addEventListener('click',(songs)=>{
    ran_song();
    Song_loop_remove();
});
